...
def calculate_revised_wilkerson_model_2025(strain_rate, dG,
    sigma0_pa=400e6, ky_sqrtm=0.28e6, E_pa=117e9, Reos_pa=22.5e9, K0_pa=140e9, rho=8960.0, N2=500e17, N0_GB=1e18, d0_G=100e-6):
    """
    Revised Wilkerson model (2025): Predicts spall strength for given strain rate and grain size.
    Default parameters:
        sigma0_pa=400e6, ky_sqrtm=0.28e6, E_pa=117e9, Reos_pa=22.5e9, K0_pa=140e9, rho=8960.0, N2=500e17, N0_GB=1e18, d0_G=100e-6
    """
    strain_rate_arr = np.asarray(strain_rate)
    if np.any(strain_rate_arr <= 0):
        ...
        positive_mask = strain_rate_arr > 0
        if np.any(positive_mask):
            results[positive_mask] = _calculate_spall_strength_complex(
                strain_rate_arr[positive_mask], dG, sigma0_pa, ky_sqrtm, E_pa, Reos_pa, K0_pa, rho, N2, N0_GB, d0_G)
        return results
    else:
        return _calculate_spall_strength_complex(strain_rate_arr, dG, sigma0_pa, ky_sqrtm, E_pa, Reos_pa, K0_pa, rho, N2, N0_GB, d0_G)
#=============================================================================#