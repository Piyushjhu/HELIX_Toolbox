...
class AnalysisThread(QThread):
    ...
    def analyze_spall_horizontal_plateau(self, time, velocity, uncert, density, acoustic_velocity, config):
        ...
        # DNS Check 1: P3 too close to zero (using the actual P3, not the minimum)
        if abs(v_p3) <= 10.0:
            # Calculate intersections and fits even for DNS (for visualization)
            ...
            # Need to find P4 and next minimum even for DNS case
            # P4: Global maximum after P3
            recomp_search_dns = velocity[idx_p3+1:]
            if len(recomp_search_dns) > 0:
                idx_p4_local_dns = np.argmax(recomp_search_dns)
                idx_p4_dns = idx_p3 + 1 + idx_p4_local_dns
                v_p4_dns = velocity[idx_p4_dns]
                t_p4_dns = time[idx_p4_dns]
            else:
                idx_p4_dns = idx_p3
                v_p4_dns = v_p3
                t_p4_dns = t_p3
            ...
            if len(post_p4_dns) > 0:
                ...
                if len(valleys_dns) > 0:
                    idx_next_min_dns = idx_p4_dns + 1 + valleys_dns[0]
                    t_next_min_dns = time[idx_next_min_dns]
                    v_next_min_dns = velocity[idx_next_min_dns]
                else:
                    idx_next_min_dns = len(velocity) - 1
                    t_next_min_dns = time[idx_next_min_dns]
                    v_next_min_dns = velocity[idx_next_min_dns]
            else:
                idx_next_min_dns = idx_p4_dns
                t_next_min_dns = t_p4_dns
                v_next_min_dns = v_p4_dns
            
            # Line 4: From P3 to P4
            if abs(t_p4_dns - t_p3) > 1e-6:
                m4_dns = (v_p4_dns - v_p3) / (t_p4_dns - t_p3)
                c4_dns = v_p3 - m4_dns * t_p3
            else:
                m4_dns = 0.0
                c4_dns = v_p3
            
            # Line 5: From P4 to next minimum
            if abs(t_next_min_dns - t_p4_dns) > 1e-6:
                m5_dns = (v_next_min_dns - v_p4_dns) / (t_next_min_dns - t_p4_dns)
                c5_dns = v_p4_dns - m5_dns * t_p4_dns
            else:
                m5_dns = 0.0
                c5_dns = v_p4_dns
            ...
            fits = {
                ...
                'seg4_recomp': {'m': m4_dns, 'c': c4_dns, 't_range': (t_p3, t_p4_dns), 'start_idx': idx_p3, 'end_idx': idx_p4_dns},
                'seg5_tail': {'m': m5_dns, 'c': c5_dns, 't_range': (t_p4_dns, t_next_min_dns), 'start_idx': idx_p4_dns, 'end_idx': idx_next_min_dns}
            }
            ...
            intersections = [
                ...
                (t_p4_dns, v_p4_dns)     # P4: Global maximum after P3
            ]
