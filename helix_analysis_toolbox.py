            ...
            if self.analysis_mode != "spade_only" and self.input_files:
                ...
                for i, input_file in enumerate(files_to_process):
                    ...
                    if smart_selection_enabled and self.analysis_mode == "both":
                        # For combined mode, only save the file needed for SPADE
                        ...
                        alpss_params['save_velocity_smooth_uncert_csv'] = True  # Only this file is needed for SPADE
                        alpss_params['save_results_csv'] = False
                        alpss_params['save_noise_csv'] = False
                    else:
                    ...
                    try:
                        ...
                        required_files = []
                        if smart_selection_enabled and self.analysis_mode == "both":
                            # For combined mode with smart selection, only check for SPADE input file
...
class HELIXAnalysisToolbox(QMainWindow):
    ...
    def create_alpss_params_tab(self):
        ...
        self.smart_selection_checkbox.setToolTip("When ALPSS+SPADE mode is selected, automatically save only files needed for SPADE analysis")
        ...
        self.smart_select_output_files.setToolTip("Select only files needed for SPADE analysis")
    ...
    def smart_select_output_files_func(self):
        """Smart select only files needed for SPADE analysis"""
        # For SPADE analysis, we only need the smoothed velocity with uncertainty file
        self.save_velocity_csv.setChecked(False)
        self.save_velocity_smooth_csv.setChecked(False)
        self.save_velocity_uncert_csv.setChecked(False)
        self.save_velocity_smooth_uncert_csv.setChecked(True)  # This is the only file SPADE needs
        self.save_results_csv.setChecked(False)
        self.save_noise_csv.setChecked(False)
